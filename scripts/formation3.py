...
def align(follower,follower_num,leader,leader_num,direction):
    ...
    if (direction=='x'):
        ...
        rospy.loginfo('Align Axis for Robot [{}] is now: {}'.format(leader_num+1,align_axis_leader))
...
def followers_routine_step2(follower_id,neighbor_id,next_follower_id):
    ...
    if (neighbor_id != 55):
        ...
        #procedure:
        if ((follower_neighbor_x < follower_neighbor_y) and align_axis[follower_id][1]==0):
            align(poses[neighbor_id], neighbor_id ,next_goal[follower_id],follower_id ,'y' )

        elif((follower_neighbor_x > follower_neighbor_y) and align_axis[follower_id][0]==0):
